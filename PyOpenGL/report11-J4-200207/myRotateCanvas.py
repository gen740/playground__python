...
class MyRotateCanvas(MyGLCanvas):
    ...
    def motion(self, x, y):
        if self.buttondown == GLUT_LEFT_BUTTON: 
            ...
            self.x, self.y = (x, y)
            # alpha = 2*np.pi*self.rotX/360
            # thetaZ = -thetaY * np.sin(alpha) 
            # thetaY = thetaX * np.sin(alpha) + thetaY * np.cos(alpha) 
            # self.rotX, self.rotY, self.rotZ = (self.rotX+thetaX, self.rotY+thetaY, self.rotZ+thetaZ)
            self.display()
